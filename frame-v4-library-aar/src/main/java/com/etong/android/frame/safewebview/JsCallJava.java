...
public class JsCallJava {
	...
	public JsCallJava (String injectedName, Class injectedCls) {
        try {
            ...
            Logger.t(TAG).e( "init js error:" + e.getMessage());
        }
    }
	...
	private String genJavaMethodSign (Method method) {
        ...
        if (len < 1 || argsTypes[0] != WebView.class) {
            Logger.t(TAG).w( "method(" + sign + ") must use webview to be first parameter, will be pass");
            return null;
        }
        for (int k = 1; k < len; k++) {
    ...
    private String getReturn (String reqJson, int stateCode, Object result) {
        ...
        Logger.t(TAG).d( mInjectedName + " call json: " + reqJson + " result:" + resStr);
        return resStr;
    }
}